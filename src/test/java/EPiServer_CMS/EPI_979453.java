...
* Author        : Amrata M.
*
* Test case     : Verify removal of selected contents from translation job
* Preconditions : This test case verifies the removal of selected contents from translation job
* 
********************************************************************************************************************************/
public class EPI_979453 {
	Boolean assertion = true;
	LinkedHashMap<String, String> dataSet = new LinkedHashMap<String, String>();
	
	String SubName = "EpiServer_Sub_979453";
	String NofromURL="";
	@BeforeMethod
	public void setUp() throws Exception{
		BrowserFactory.SystemEngine().startApplication("EPI_979453");
		dataSet.put("TL_test_case_description","EPI_979453: Verify removal of selected contents from translation job");
		dataSet.put("TL_internal_testCase_ID", "979453");
		...
		public void testcase(LinkedHashMap<String, String> dataSet) throws Exception{
			try{	
				...
				//Get the Number from URL
				 NofromURL=common.action().GetNofromURL();
				...
				assertion= Verify.action().verifyElementPresent(locators.exts().CMS_new_job_Content_page("Alloy Meet ",NofromURL), 10);	
				if(assertion == false){
					report("f"," assertion is failed while verifying Selected Page Number");
					
				}
								
				BrowserFactory.SystemEngine().findElement(locators.exts().GL_Content_CustomReferences).click();
				Thread.sleep(3000);
		...
		try{
			assertion= Verify.action().verifyElementNotPresent(locators.exts().CMS_new_job_Content_page("Alloy Meet ",NofromURL), 10);	
			if(assertion == false){
				report("f"," assertion is failed while verifying Selected Page Number");
			}
			else{
