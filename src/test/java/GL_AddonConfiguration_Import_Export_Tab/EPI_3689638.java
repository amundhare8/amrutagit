...
* Author        : Amrata M.
*
* Test case     : Verify whether the functionality for Simple Address when user selects From latest target option from drop down
* Precondition 	: Set the value of 'Copy 'Simple address'' to 'From latest target' in Export/Import tab

                :- For steps 1,2 and 3 - submit a job for DE-de and retrieve it with Copy From Source option (so that the target version should have simple address)
 * Summary      : This tc is to verify 'Simple Address' functionality when selected 'From latest target' option


*******************************************************************************************************************************************************************************************/
public class EPI_3689638 {
	
	Boolean assertion = true;
	LinkedHashMap<String, String> dataSet = new LinkedHashMap<String, String>();
	String PageName = "Epi_3689638";
	String SA="SATEST@1";
	String SA2="SATEST";
	@BeforeMethod
	public void setUp() throws Exception{
		
		BrowserFactory.SystemEngine().startApplication("EPI_3689638");
		          //  Test case description here
				dataSet.put("TL_test_case_description","EPI_3689638: Verify whether the functionality for Simple Address when user selects From latest target option from drop down.");
				dataSet.put("TL_internal_testCase_ID", "3689638");
	}	
		public void testcase(LinkedHashMap<String, String> dataSet) throws Exception{
			try{
				...
				common.action().Inside_ExportImportTab_Functionality("GlobalLink", "Source", "From source", "Create copy from source", "Cancel it", "Not Ready (Draft)", "Skip","From source");
			    ...
			    //de-de
			    assertion=Verify.action().verifyElementPresent(locators.exts().CMS_edit_submission_NewSubmission_SA_ChangeModified(SA), 10);
		    	if(assertion == false){
					report("f"," assertion is failed while verifying Simple Address Value should be copied from source.");							
		    	}
				...
				//Inside GlobalLink Export/Import
				common.action().Inside_ExportImportTab_Functionality("GlobalLink", "Source", "From source", "Create copy from source", "Cancel it", "Primary Draft", "Skip","From latest target");
			    ...
			    //Modifying 
				common.action().ModifingFields_NewPage("2","Alloy - collaboration,","Alloy Plan, Alloy Meet, Alloy Track, distance collaboration,","Alloy solves the two most pressing problems in long distance collaboration","None");
				...
				Thread.sleep(2000);
				 ...
				 //de-de
			    assertion=Verify.action().verifyElementPresent(locators.exts().CMS_edit_submission_NewSubmission_SA_ChangeModified(SA2), 10);
		    	if(assertion == false){
					report("f"," assertion is failed while verifying Simple Address Value should be copied from source.");							
		    	}
		...
		try{//Edit_Gl_draftDemo
			assertion=Verify.action().verifyElementPresent(locators.exts().Edit_Gl_draftDemo, 10);
	    	if(assertion == false){
