...
* Author        : Amrata M.
*
* Test case     : Verify whether reference depth is added on submission window USE CASE 2
* 
...
* 
***********************************************************************************************/
public class EPI_4881278 {

	Boolean assertion = true;
	LinkedHashMap<String, String> dataSet = new LinkedHashMap<String, String>();
	
	String SubName = "EpiServer_Sub_4881278";
	String ProjectName= "EPiServer";
	int SelectNum;
	
	@BeforeMethod
	public void setUp() throws Exception{
		BrowserFactory.SystemEngine().startApplication("EPI_4881278");
		dataSet.put("TL_test_case_description","EPI_4881278: Verify whether reference depth is added on submission window USE CASE 2.");
		dataSet.put("TL_internal_testCase_ID", "4881278");
		...
		public void testcase(LinkedHashMap<String, String> dataSet) throws Exception{
			try{	
				...
				BrowserFactory.SystemEngine().findElement(locators.exts().RefDepth_Input1).sendKeys("4");
				Thread.sleep(2000);				
				...
				Thread.sleep(3000);
				//
				common.action().create_basic_submission_MaxFiles(SubName,false,true, true, true, false,true, "4", false, false, true, true, true, "None");
		...
		try{
			//1
			assertion=Verify.action().verifyElementPresent(locators.exts().SubmitTranslation_ContentPreview_SubTree1, 10);				
			Thread.sleep(2000);
			...
			//3
			assertion=Verify.action().verifyElementPresent(locators.exts().SubmitTranslation_ContentPreview_SubTree2, 10);				
			Thread.sleep(2000);
			System.out.println("Flag=="+assertion);
			if(assertion == false){
				report("f"," assertion is failed while verifying Referenced Content SubTree.");
			...
			//4
			assertion=Verify.action().verifyElementPresent(locators.exts().SubmitTranslation_ContentPreview_SubTree4, 10);				
			Thread.sleep(2000);
			System.out.println("Flag=="+assertion);
			if(assertion == false){
				report("f"," assertion is failed while verifying Referenced Content SubTree.");
