 ...
 * Author 	 : Amrata M.
 *
 * Test case : Verify 'Include Expired Source' Configuration option on translation job page
 * 
 * Summary   : This test case verifies the functionality of 'Include Expired Source' Configuration option on translation job page

 Precondition : Create a Page and Expire the source content page
 *
 * 
 *******************************************************************************************************************************************************************/
public class EPI_979456 {
	Boolean assertion = true;
	LinkedHashMap<String, String> dataSet = new LinkedHashMap<String, String>();
	
	String SubName = "EpiServer_Sub_979456";
	String ProjectName= "EPiServer";
	
	@BeforeMethod
	public void setUp() throws Exception{
		BrowserFactory.SystemEngine().startApplication("EPI_979456");
		dataSet.put("TL_test_case_description","EPI_979456:  Verify 'Include Expired Source' Configuration option on translation job page");
		dataSet.put("TL_internal_testCase_ID", "979456");
		...
		public void testcase(LinkedHashMap<String, String> dataSet) throws Exception{
			try{	
				...
				BrowserFactory.SystemEngine().findElement(locators.exts().Edit_Selectpage_howtobuy).click();
				Thread.sleep(3000);
				
				BrowserFactory.SystemEngine().findElement(locators.exts().Select_page("Find a reseller")).click();
				Thread.sleep(5000);
				...
				common.action().Config_ReferencedContentTypesChecks_WithDepthAndActions(true, "2", true, false, true, true, true, "None");
		    	...
		    	if ( !BrowserFactory.SystemEngine().findElement(locators.exts().Gl_Submit_Translation_IncludeExpired("Select Expired Source")).isSelected() )
				{
					BrowserFactory.SystemEngine().findElement(locators.exts().Gl_Submit_Translation_IncludeExpired("Select Expired Source")).click();
				}
				Thread.sleep(2000);
				...
				assertion= Verify.action().verifyElementPresent(locators.exts().Submit_Translation_Contentpreview("Find a reseller"), 10);	
				if(assertion == false){
		...
		try {
			...
			Thread.sleep(2000);
			//Remove_Expiration
			BrowserFactory.SystemEngine().findElement(locators.exts().CMS_Job_Tool_arrow_Remove_Expiration).click();
			Thread.sleep(3000);
